...
## When to use
- After completing a build or any significant code change
- Before wrapping a session
- Any time you want a clean, well-organized git history
...
- **CRITICAL: Tests and docs are the source of truth — not the code.**
  If code changes cause test failures, the default assumption is the code is wrong,
  not the tests. Never change a test just to make it pass after a code change without
  explicit developer review and sign-off. Test logic changes require the developer
  to confirm the new behaviour is correct and intentional. Flag any test change
  in the commit plan and wait for confirmation before proceeding.
