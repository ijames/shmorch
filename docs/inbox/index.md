...
`session-wrote-git-history-in-another-repo.md`, filed from `treeclusion` 2026-09-24. A session rooted in the Shmorch repo rebased branches, opened and merged a PR, and left a conflicted rebase in detached HEAD inside `treeclusion`'s checkout — while a second agent was live in that same checkout and nearly committed into the conflicted rebase. To investigate: why a SELF-repo session targeted another project's working tree, and whether git writes should be pinned to the session's own repo plus a liveness check before touching a checkout. Adjacent to `agent-window-confusion.md`, from the other direction.
