↑ [Inbox](index.md)
**In this section:** [Ambient/system-triggered state changes need smooth transitions, not just user-initiated ones](ambient-system-triggered-transitions.md) · [Backlog may be better modeled as a dependency stack than a flat list](backlog-dependency-stack.md) · [Command → workflow → role/tools audit](command-workflow-role-audit.md) · [Product design criteria: respect system settings by default](product-design-respect-system-settings.md) · [Stub tracks on separate branches collide on shared index/plan files](stub-tracks-shared-file-collision.md)
