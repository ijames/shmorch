...
|---|---|---|---|---|
| 2026-09-23 | 2.3.5 | A node must read as a complete document of its kind with every link stripped — no sentence whose claim only resolves by following a link, no promise a child doc keeps instead. See `core/documentation.md` § Progressive Disclosure, depth read test | `additive` | No backfill offered — existing trees remain valid; apply going forward and opportunistically when a tree is next touched |
| 2026-09-21 | 2.3.4 | Progressive Disclosure is recursive, not just an index-vs-leaf split — every node in an AI-managed doc tree (not only the root/index) must stand at its own depth: lede first, quantities roll up to parents, redundancy between parent/child is intended, leaves hold detail only. See `core/documentation.md` § Progressive Disclosure, depth read test | `additive` | No backfill offered — existing trees remain valid; apply going forward and opportunistically when a tree is next touched |
