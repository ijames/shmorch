...
**Decisions index growth:** a pre-split `decisions/index.md` is chronological and additive — entries are never removed; a superseded decision gets a correction note, not a rewrite (see `templates/docs/{product,technology}/decisions/index.md`). That changes once it accumulates many entries (rule of thumb: high entry count, or multiple entries that supersede/correct earlier entries on the same topic): split it into `docs/product/decisions/<topic>.md` or `docs/technology/decisions/<topic>.md` files by topic (e.g. `stack.md`, `process.md`, `data-architecture.md`, `ux-motion.md`, `infra-ops.md`, `product-monetization.md` — topics follow the project's actual decision clusters, not a fixed list). Rewrite `decisions/index.md` itself into a short index linking to each topic file. Each topic file states only the *current* form of each decision, never the history of how it was reached or revised — collapse correction chains into one clean statement. This is a deliberate trade at split time, not a contradiction of the pre-split rule above: git log / commit messages become the audit trail for how a decision evolved once its topic file compacts, since the index and its topic files no longer are. Reference implementation (pre-restructure naming): DarkBadge's `docs/development/decisions/` split (2026-06-19).

**Decisions vs. directives:** a `decisions/` entry is a passive historical record — writing it is not the same as acting on it. A decision that changes workflow behavior, role behavior, or code functionality (a *directive*) isn't done until the active node that implements it (a `workflows/*.md` step, a `core/*.md` rule, `agents/**`, or the code itself) is actually updated to carry it out — link back to the decision-log entry for rationale rather than restating it inline, so the decision's context loads narrowly on demand instead of ballooning the active file. A decisions-log entry describing a behavior change with no corresponding edit to any active file is incomplete, not just unenforced.
