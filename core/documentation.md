...
This doesn't change the graduation model — a track's *knowledge* still only leaves `docs/state/tracks/` for `docs/<category>/` when the track closes (see Graduation rule above). This split keeps an *open, still-growing* track's own footprint from becoming a token liability while it's being read for orientation, nothing more.
