...
`docs/reference/learning/multi-agent-role-composition.md`. Name the session after that role so it can be resumed instead of re-spawned. Use a model **tier**, never a vendor name: `default` for routine roles, `strong` for the critic.
